...
6. After the slice code and tests land: run `bash scripts/run-tests.sh` (headless). Do not start a Godot window. Then set the slice status to `implementiert`. Do **not** mark a multi-phase stage as `erledigt` (parent does that after headless tests, review, and push; visual playtest only on request). If the INDEX has a Fortschritt table, you may set the current stage to `in Umsetzung` if it was still `offen`.
