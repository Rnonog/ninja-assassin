class_name ClanCaptain
extends CharacterBody2D

enum Phase { CHASE, TELEGRAPH, ACTIVE, PAUSE, DEAD }

const CAPTAIN_SPEED := 110.0
const STOMP_RANGE := 56.0
const STOMP_TELEGRAPH := 0.55
const STOMP_ACTIVE := 0.18
const STOMP_PAUSE := 0.9
const HITBOX_SIZE := Vector2(96, 40)
const HITBOX_FORWARD := 0.0
const COLOR_IDLE := Color(0.72, 0.12, 0.18, 1)
const COLOR_BAND := Color(0.95, 0.78, 0.22, 1)
const COLOR_OUTLINE := Color(0.96, 0.95, 0.9, 1)
const COLOR_TELEGRAPH := Color(0.92, 0.72, 0.12, 1)
const COLOR_STOMP := Color(0.78, 0.14, 0.12, 1)
const OUTLINE_GROW := 4.0
const BODY_SIZE := Vector2(32, 56)

@export var auto_aggro: bool = false
...
func _ready() -> void:
	...
	if health:
		health.max_hp = Health.BOSS_MAX_HP
		health.current = Health.BOSS_MAX_HP
	...
	if _hitbox:
		...
		_hitbox.team = &"enemy"
		_hitbox.damage = Health.BOSS_STOMP_DAMAGE
...
func _tick_chase(_delta: float) -> void:
	...
	_face_player()
	var dx := target.global_position.x - global_position.x
	if absf(dx) <= STOMP_RANGE:
		velocity.x = 0.0
		request_stomp()
		return
	velocity.x = signf(dx) * CAPTAIN_SPEED
...
func _sync_hitbox_transform() -> void:
	if _hitbox == null:
		return
	_hitbox.position = Vector2(HITBOX_FORWARD, 8.0)
